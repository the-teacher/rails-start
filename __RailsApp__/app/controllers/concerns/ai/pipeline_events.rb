module Ai
  module PipelineEvents
    ...
    def pipeline_step_start_event(step_name, step_index)
      {
        event:  "step_start",
        step:   step_name,
        index:  step_index,
        label:  step_label(step_name),
        text:   "→ #{step_label(step_name)}: #{STEP_DESCRIPTIONS[step_name] || step_name}",
        level:  "info",
        source: "pipeline"
      }
    end

    def pipeline_step_done_event(step_name, result)
    ...
    def pipeline_stopped_event(step_name, result)
      {
        event:  "stopped",
        step:   step_name,
        text:   "✗ Stopped at #{step_label(step_name)} — #{stop_reason_text(step_name, result)}",
        level:  "error",
        source: "pipeline"
      }
    end
    ...
    def step_label(step_name)
      STEP_LABELS[step_name] || step_name.to_s
    end
