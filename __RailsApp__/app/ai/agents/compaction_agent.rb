...
class CompactionAgent < ActiveHarness::Agent
  ...
  before(:call)      { Rails.logger.info  "[Compaction] ▶ calling…" }
  after(:call)       { |r| Rails.logger.info  "[Compaction] ✓ done (#{r.execution_time}s) — #{r.output.to_s.truncate(80)}" }
  callback(:retry)   { |entry, err| Rails.logger.warn  "[Compaction] ↺ retry #{entry&.dig(:model)} — #{err&.message}" }
  callback(:failure) { Rails.logger.error "[Compaction] ✗ all models failed" }
