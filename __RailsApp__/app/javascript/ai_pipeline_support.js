...
function plSetStep(stepName, state) {
  ...
  if (!el) return;
  el.className = "ah-pl-step ah-pl-step--" + state;
  icon.textContent = STEP_ICONS[state] || "○";
}
...
function plAppendLog(text, level, source) {
  ...
  if (source && SOURCE_LABELS[source]) {
    badgeHtml =
      '<span class="ah-pl-source-badge ah-pl-badge--' +
      source +
      '">' +
  ...
  line.innerHTML =
    '<span class="ah-pl-log-icon">' +
    (LOG_ICONS[lvl] || "●") +
    "</span>" +
    badgeHtml +
    '<span class="ah-pl-log-text">' +
    text +
    "</span>";
...
function plResetOutput() {
  var out = document.getElementById("ah-pl-output");
  var meta = document.getElementById("ah-pl-output-meta");
  out.textContent = "—";
  meta.textContent = "";
}
...
(function () {
  ...
  form.addEventListener("submit", function (e) {
    ...
    var es = new EventSource(
      "/ai/pipelines/support/stream?input=" + encodeURIComponent(val),
    );
    var activeStep = null;
    ...
    es.addEventListener("completion", function (ev) {
      ...
      if (p.error) {
        ...
        plAppendLog(
          "Pipeline stopped at: " + p.stopped_at,
          "error",
          "pipeline",
        );
      } else {
        ...
        if (p.time) meta.textContent = "Total: " + p.time + "s";
      }

      closeStream();
    });

    // ── All SSE events (pipeline + tribunal + agent all come on same channel) ─
    es.addEventListener("processing", function (ev) {
      ...
      if (p.event === "step_start") {
        ...
        plAppendLog(p.text, "info", "pipeline");
      } else if (p.event === "step_done") {
        plSetStep(p.step, "done");
        plAppendLog(p.text, "success", "pipeline");
      } else if (p.event === "stopped") {
        if (activeStep) plSetStep(activeStep, "stopped");
