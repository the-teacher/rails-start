...
base-images-arch-check:
	...
	@ARCH=$$(docker run --rm --platform linux/arm64 $(DOCKER_HUB_REPO):latest uname -m); \
		echo -n "$$ARCH"; \
		if [ "$$ARCH" = "aarch64" ]; then echo " ✓"; else echo " ✗ MISMATCH!"; fi
	@echo ""
	@echo "Docker Hub (amd64):"
	@printf "  Expected: x86_64 | Actual: "
	@ARCH=$$(docker run --rm --platform linux/amd64 $(DOCKER_HUB_REPO):latest uname -m); \
		echo -n "$$ARCH"; \
		if [ "$$ARCH" = "x86_64" ]; then echo " ✓"; else echo " ✗ MISMATCH!"; fi
	@echo ""
	@echo "GitHub Container Registry (arm64):"
	@printf "  Expected: aarch64 | Actual: "
	@ARCH=$$(docker run --rm --platform linux/arm64 $(GITHUB_REPO):latest uname -m); \
		echo -n "$$ARCH"; \
		if [ "$$ARCH" = "aarch64" ]; then echo " ✓"; else echo " ✗ MISMATCH!"; fi
	@echo ""
	@echo "GitHub Container Registry (amd64):"
	@printf "  Expected: x86_64 | Actual: "
	@ARCH=$$(docker run --rm --platform linux/amd64 $(GITHUB_REPO):latest uname -m); \
		echo -n "$$ARCH"; \
		if [ "$$ARCH" = "x86_64" ]; then echo " ✓"; else echo " ✗ MISMATCH!"; fi
	@echo ""
	@echo "=============================================================="
